...
export function buildBrief(cluster: LiveCluster, articles: LiveArticle[], opts: { now?: number } = {}): IFFABrief {
  const raw = synthesizeBrief(cluster, articles, { language: "en", now: opts.now });
...
export function buildBriefs(cluster: LiveCluster, articles: LiveArticle[], opts: BuildBriefOptions = {}): BuiltBriefs {
  const en = buildBrief(cluster, articles, { now: opts.now });
  const wantTamil =
...
export function microBrief(cluster: LiveCluster, articles: LiveArticle[], opts: { now?: number } = {}): MicroBrief {
